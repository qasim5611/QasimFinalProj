...
import { sendToActionFORRetriveTchInfo } from './../Redux/Action/action';
import { DeleteTchRecordFromStore } from './../Redux/Action/action';
...
export class SDashBoard extends Component {
    ...
    notifyB = (a) => toast.success('Request have been sent to Sir, '+a+' Via Msg Api', {containerId: 'B'});
  // state = {
  //   loading: true
  // }
  ...
  componentWillMount() {
    ...
    this.props.sendToActionFORRetriveTchInfo();
    // DateMangwaLiya();
  ...
  DeleteTchRecordFromStore = (id) => {
    ...
    this.props.DeleteTchRecordFromStore(data);
            ...
            <body>
              <Row>
                ...
                <Col xs={12} sm={12} lg={9} md={9} style={{ backgroundColor: 'orange', height: "150px", float: "left", backgroundColor: "Black", borderBottomRightRadius: "200px" }}>
                  <center><p style={{ paddingTop: "30px", paddingLeft: "50px", color: "white", fontFamily: "cursive", fontSize: "40px" }} >Student DashBoard</p></center>
                </Col>
                  ...
                  <div style={{ height: "400px", width: "px", backgroundColor: "", overflow: "scroll" }}>
                    <table style={{ border: "2px black dashed", width: "px" }}>
                      <tr style={{ border: "2px black solid", width: "px" }} >
                        <th style={{ border: "2px black solid", width: "px" }}>Teacher-ID</th>
                        ...
                        <th style={{ border: "2px black solid", width: "px" }}>Qualification/Major Subjects</th>
                        ...
                        this.props.data.map((value, index) => {
                          ...
                          return (
                            ...
                            <tr key={index} style={{ border: "2px black dashed", width: "px" }}>
                              ...
                              <td style={{ paddingLeft: "px",border: "2px black dashed" }} id="dic1"> {value.qul}</td>
                              ...
                              <td >
                                ...
                                <Button variant="contained" color="secondary"  onClick={() => this.DeleteTchRecordFromStore(value._id)} >
        Delete
        <DeleteIcon/>
      </Button>
...
const mapStateToProps = (store) => {
  console.log('+++++++++++state++++++++++++++++', store.ReducerForDBdata.TchInfo);

  return {
    // rtitle : store.title,
    // rDiscription:  store.descp,
  data: store.ReducerForDBdata.TchInfo
  //data: store.ReducerForDBdata.StateForFirebase   For Firbase DB case

  }
}

export default connect(mapStateToProps, { sendToActionFORRetriveTchInfo,DeleteTchRecordFromStore})(SDashBoard);